...
class Game_Round:
    def __init__(self, game_players, pool_obj, step_func_obj, log=True):
        ...
        self.game_rounds = [
            [self.round_1],
            [self.combat_round],[self.minion_round],
            [self.combat_round],[self.minion_round],
            [self.combat_round],
            [self.combat_round],
            [self.combat_round],
            [self.combat_round],[self.carousel_round, self.combat_round],
            [self.combat_round],
            [self.combat_round],[self.minion_round],
            [self.combat_round],
            [self.combat_round],
    ...
    def decide_player_combat(self):
        ...
        while len(player_list) >= 2:
            ...
            player.opponent_options = {player_id: 0 for player_id in self.PLAYERS.keys()}
            ...
            opposition.opponent_options = {player_id: 0 for player_id in self.PLAYERS.keys()}
        ...
        if len(player_list) == 1:   # if there is only one player left to match, have it fight a ghost
            ...
            self.PLAYERS[player_list[0]].opponent_options = \
                {player_id: 0 for player_id in range(config.NUM_PLAYERS)}
