...
def _log(config, step_count, log_data, model, replay_buffer, lr, shared_storage, summary_writer, vis_result):
    ...
    total_loss, weighted_loss, loss, reg_loss, policy_loss, value_prefix_loss, value_loss, consistency_loss, chance_loss, commitment_loss = loss_data
    if vis_result:
    ...
    _msg = '#{:<10} Total Loss: {:<8.3f} [weighted Loss:{:<8.3f} Policy Loss: {:<8.3f} Value Loss: {:<8.3f} ' \
           ...
           'Chance Loss: {:<8.3f} Commitment Loss: {:<8.3f} ] ' \
    ...
    _msg = _msg.format(step_count, total_loss, weighted_loss, policy_loss, value_loss, value_prefix_loss, consistency_loss,
                       chance_loss, commitment_loss, replay_episodes_collected, replay_buffer_size, total_num / 1000, config.batch_size, lr)
